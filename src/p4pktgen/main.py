...
def main():
    ...
    if args.format == 'json':
        ...
        ser_timer.stop()
        if json_cmp(json_a='test-cases-ser.json', json_b='test-cases-par.json'):
            logging.info('JSON comparison succeeded')
        else:
            logging.error('Mimatch between Serial/Parallel JSON')
        # Or compute speedup ?
        print('Parallel Code Time: %.3f sec' %
                     (par_timer.get_time()))
...
def json_cmp(json_a, json_b):
    ...
    fh_a = open(json_a)
    fh_b = open(json_b)
    json_obj_a = json.load(fh_a)
    json_obj_b = json.load(fh_b)
    fh_a.close()
    fh_b.close()
    ...
    for it_a in json_obj_a:
        ...
        for idx, it_b in enumerate(json_obj_b):
            # The comparison should work as long as the keys/valus in the dict are hashable, which they should be since
        ...
        while True:
            try:
                ...
                for tc in test_case.test_cases:
                    final_case_writer.write(tc, [])
                final_case_writer.write(None, test_case.packet_lst)
