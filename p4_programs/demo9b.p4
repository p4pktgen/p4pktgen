...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    apply {
        // Note that it is definitely possible that hdr.ipv6.version
        // is being accessed without it being initialized here,
        // e.g. if there is no IPv6 header in the packet.  p4pktgen
        // should not crash while analyzing such a program, but it
        // probably should be able to warn the user of a particular
        // input packet (and later also table contents) that causes
        // the program to read such an uninitialized header field.
