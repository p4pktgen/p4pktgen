...
class Node(object):
    ...
    def print_(self):
        print "node", self, "edges:", ", ".join([str(n) for n in self.edges])
        for n in self.edges:
            n.print_()
...
class Graph(object):
    ...
    def produce_topo_sorting(self):
        def visit(node, topo_sorting, sequence = None):
            if sequence is not None: sequence += [str(node)]
            if node._behavioral_topo_sorting_mark == 1:
                if sequence is not None: print "cycle", sequence
                return False
            if node._behavioral_topo_sorting_mark != 2:
                node._behavioral_topo_sorting_mark = 1
                for next_node in node.edges:
                    if sequence is not None:
                        res = visit(next_node, topo_sorting, list(sequence))
                    else:
                        res = visit(next_node, topo_sorting)
